...
def getOrderedScore(positions, board, humans_memory, ennemies_memory):
    ...
    for x, y, pos_nb, _, _ in positions:
        ...
        pos_storage.append([x,y]) #store positions we loop through (= number of splits considered)
        ...
        if (x, y, pos_nb) not in ennemies_memory:
            if len(board.getOpponentDict()) > 0 :
                ...
                ennemy = np.array(list(board.getOpponentDict().keys())) # all enemy positions 
                ...
                for dist, enemy_unit in zip(distanceEnemies, board.getOpponentDict().values()):
                    if (0.5 * pos_nb < enemy_unit): de.append(dist) # enemy that could kill us 
    ...
    if s < nb_units/15 and len(pos_storage) > 1 : # if a few humans are left
        print('pos storage', pos_storage)
        ...
        bonus += 1000000 / intra_dist * len(pos_storage)

    # Other part of the merge incentive 
    """  
    # If the enemy is close to us, give incentive to regroup 
    if (min_de != []):
        if min(min_de) < 2 and len(pos_storage)> 1: 
            ...
            bonus -= 10 * intra_dist * len(pos_storage) 
...
def minimax(current_board, exploredPositions, heuristicHistory, clock, depth = DEPTH, 
    ...
    print('fed into SmartScan', truncatedPossibleMoves)
